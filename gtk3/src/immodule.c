...
static const guint SKIP_MASK = GDK_MOD1_MASK | GDK_MOD2_MASK | GDK_MOD3_MASK | GDK_MOD4_MASK | GDK_MOD5_MASK;
...
#define KIME_IM_CONTEXT(var) \
  KimeImContext *ctx = G_TYPE_CHECK_INSTANCE_CAST(var, KIME_TYPE_IM_CONTEXT, KimeImContext)
...
gboolean filter_keypress(GtkIMContext *im, GdkEventKey *key) {
    ...
    else
    {
        ...
        InputResult ret = kime_engine_press_key(ctx->engine, ctx->config, key->hardware_keycode, state);
...
void get_preedit_string(GtkIMContext *im, gchar **out, PangoAttrList **attrs, int *cursor_pos)
{
    KIME_IM_CONTEXT(im);
    uint32_t c = 0;
    size_t str_len = 0;
...
#define SET_SIGNAL(NAME)                              \
    klass->signals.NAME = g_signal_lookup(#NAME, KIME_TYPE_IM_CONTEXT); \
    g_assert(klass->signals.NAME != 0);
    SET_SIGNAL(commit);
    SET_SIGNAL(preedit_start);
    SET_SIGNAL(preedit_changed);
    SET_SIGNAL(preedit_end);
#undef SET_SIGNAL
